...
const saveTrades = async ({
  ...
  const filteredResults = _.orderBy(
    trades?.filter(
      (i) =>
        !existingExecutions.includes(i.orderId) &&
        ["stocks", "option", "forex", "future", "crypto"].includes(i.assetClass)
    ),
    [(i) => dayjs(i.date).valueOf()], // sort by timestamp
    ["asc"] // ascending order
  );
  let allOptions = [];
  let allStocks = [];
  ...
  return {
    importedTrades: filteredResults?.length,
    totalTrades: trades?.length,
    duplicateTrades: trades?.length - filteredResults?.length,
  };
};
