...
const syncEtrade = require("../../utils/brokerSync/Etrade/syncEtrade");
const fetchQuesTradeData = require("../../services/brokerSync/QT/fetchQTToken/fetchQuesTradeData");
const syncQT = require("../../utils/brokerSync/QT/syncQT");
  ...
  try {
    ...
    //Write a condition for mt4 or mt5 here
      ...
      await syncEtrade({
        ...
        ...(isNotFirstSync
          ? {
              fromDate: dayjs(latestExecution?.date)
                .subtract(2, "days")
                .format("MMDDYYYY"),
            }
          : {
              fromDate: dayjs(broker?.details?.startDate).format("MMDDYYYY"),
            }),
      });
      ...
      await syncQT({
        ...
        importVia: "brokerSync",
        timeZone: userDetails?.timeZone,
        tokenType: broker?.details?.token_type,
        ...(isNotFirstSync
          ? {
              startTime: dayjs(latestExecution?.date)
                .subtract(2, "days")
          ...
          : {
              startTime: broker?.details?.startDate
                ? dayjs(broker?.details?.startDate).format("YYYY-MM-DD")
    ...
    if (
      ...
      (errorDetails?.message === "Error: Invalid access token used" ||
        errorDetails?.message === "Error: Token rejected, please reconnect")
    ) {
      broker.isDisconnected = true;
    }
