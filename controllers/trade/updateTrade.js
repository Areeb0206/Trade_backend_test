...
const updateTrade = async (req, res, next) => {
  try {
    ...
    trade.timeZone = timeZone;
    trade.calculationMethod = accountExist?.calculationMethod;
    ...
    // });
    const sortExecutionList = _.orderBy(executionList, ["date"], ["asc"]);
    ...
    try {
      ...
      if (tradeType === "option") {
        ...
        trade.contractMultiplier = sortExecutionList?.[0]?.contractMultiplier;
