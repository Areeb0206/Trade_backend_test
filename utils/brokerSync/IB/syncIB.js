...
const updateOldRecord = require("../../../services/brokers/ibkr/updateOldRecords");
...
const syncIB = async ({ broker, importVia, timeZone = "America/New_York" }) => {
  try {
    ...
    // );
    // const response = JSON.parse(fs.readFileSync(`3.json`));

    // update the old records , if any of the field is not updated in case of option only
    await updateOldRecord({
      data: response,
